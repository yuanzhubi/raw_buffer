...
public:
    ...
    RAWBUF_T* append(const RAWBUF_T* src, RAWBUF_A count, RAWBUF_M* offset_pointer){
        ...
        const size_t new_offset_count = RAW_BUF_ALIGN(this->data_size, rawbuf::rawbuf_property<RAWBUF_A>::alignment_result);
        const size_t new_offset = RAW_BUF_ALIGN(new_offset_count + sizeof(RAWBUF_A), align_count);
        ...
        *(RAWBUF_A*)(this->data_ptr + new_offset_count) = count;
        if(src != 0){
    ...
    RAWBUF_T* alloc(RAWBUF_A count, RAWBUF_M* offset_pointer){
        ...
        const size_t new_offset_count = RAW_BUF_ALIGN(this->data_size, rawbuf::rawbuf_property<RAWBUF_A>::alignment_result);
		//const size_t new_offset_size = RAW_BUF_ALIGN(new_offset_count + sizeof(RAWBUF_A), rawbuf::rawbuf_alignment<offset_type>::result);
        const size_t new_offset = RAW_BUF_ALIGN(new_offset_count + sizeof(RAWBUF_A), rawbuf::rawbuf_alignment<RAWBUF_T>::result);
        const size_t realsize = count * sizeof(RAWBUF_T);
        const size_t offset_diff = new_offset_count - (rawbuf_uint)((char*)offset_pointer - this->data_ptr);
        ...
        *(RAWBUF_A*)(this->data_ptr + new_offset_count) = count;
		//*(offset_type*)(this->data_ptr + new_offset_size) = sizeof(RAWBUF_T);
        RAWBUF_T* dp = (RAWBUF_T*)(this->data_ptr + new_offset);
...
struct rawbuf_builder : public rawbuf_writer_proto, public rawbuf_writer<RAWBUF_T>{
	static const rawbuf_uint align_cost = rawbuf::max_align_cost<RAWBUF_T::aligned_1x, RAWBUF_T::aligned_2x, RAWBUF_T::aligned_4x, RAWBUF_T::aligned_8x,rawbuf::rawbuf_alignment<RAWBUF_T>::result>::result;
	static const rawbuf_uint init_capacity =  rawbuf::rawbuf_property<RAWBUF_T>::size_result + align_cost ;
