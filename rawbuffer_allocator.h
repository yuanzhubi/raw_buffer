...
struct rawbuf_reader_iterator : public rawbuf_reader<T>{
    ...
    size_t element_size;
	bool element_size_check(){
		T* that = this->get();
		size_t real_size = sizeof(T) + (that->_.real_optional_fields_count - T::optional_fields_count) * sizeof(typename T::offset_type); 
		if(real_size != this->element_size){
			this->invalidate(RAW_BUF_ERROR_MSG("Invalid optional fields count!"));
			return false;
		}
		return true;
	}
	inline this_type  operator[] (size_t diff) const {this_type result = *this; result.data_ptr += diff * element_size; result.init_check() && result.element_size_check(); return result;}
    inline this_type  operator+  (size_t diff) const {this_type result = *this; result.data_ptr += diff * element_size; result.init_check() && result.element_size_check(); return result;}
    inline this_type  operator-  (size_t diff) const {this_type result = *this; result.data_ptr -= diff * element_size; result.init_check() && result.element_size_check(); return result;}

	inline this_type& operator-- ()            {this->data_ptr -= 1    * element_size; this->init_check() && this->element_size_check(); return *this;}
    inline this_type& operator-= (size_t diff) {this->data_ptr -= diff * element_size; this->init_check() && this->element_size_check(); return *this;}
