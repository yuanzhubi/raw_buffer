...
public:\
    ...
    struct rawbuf_reader_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name)> : public rawbuf_reader_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name) - 1 >{\
        rawbuf_input_type* rawbuf_name() { \
            ...
            if(!RAW_BUF_IS_ALIGNED_PTR(result, rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result))	    /* 2.The location of the optional field should be aligned.*/{	\
...
public:\
    ...
    struct members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), true > : public members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), false >{ \
		...
		static const rawbuf_uint required_alignment = (parent_type::required_alignment > rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result)  ?  parent_type::required_alignment : rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result; \
    }; \
...
#define ADD_FIELD_ARRAY(rawbuf_input_type, rawbuf_name, size_estimation) \
    ...
    struct members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), true > : public members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), false >{ \
        ...
        static void iterate(RAWBUF_This& this_instance, RAWBUF_Func& func_instance){ \
			...
			if(rawbuf_psize != 0) {\
                rawbuf_input_type* rawbuf_data = (rawbuf_input_type*)(RAW_BUF_ALIGN_TYPE((size_t)(rawbuf_psize + 1), rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result, size_t)); \
        ...
        static void copy(const RAWBUF_M& the_src, const rawbuf_writer<RAWBUF_T>& rawbuf_packet){ \
			...
			if(rawbuf_psize != 0){ \
				 rawbuf_input_type* ptr = (rawbuf_input_type*)(RAW_BUF_ALIGN_TYPE(((size_t)(rawbuf_psize + 1)), \
							rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result, size_t)); \
            ...
            if(*rawbuf_foffset != 0){ \
                ...
                if(*old_size > the_array_size){ \
                    *old_size = the_array_size; \
                    return (rawbuf_input_type*)memcpy((rawbuf_input_type*)(RAW_BUF_ALIGN_TYPE(((size_t)(old_size + 1)), rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result, size_t)) \
    ...
    struct rawbuf_reader_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name)> : public rawbuf_reader_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name) - 1 >{\
        ...
        rawbuf_input_type* rawbuf_name() { \
            ...
            rawbuf_input_type* result = (rawbuf_input_type*)(RAW_BUF_ALIGN_TYPE(((size_t)(rawbuf_psize + 1)), rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result, size_t)); \
...
#define ADD_PACKET_COMMON(rawbuf_input_type, rawbuf_name, init_func) \
    ...
    struct members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), true > : public members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), false >{ \
        ...
        static void iterate_with_depth(RAWBUF_This& this_instance, RAWBUF_Func& func_instance, int rawbuf_depth){ \
            ...
            if(instance != 0 && result){ \
				instance->iterate_with_depth(func_instance, rawbuf_depth + 1); \
            }\
			if(next_type::is_required || this_instance._.real_optional_fields_count > RAW_BUF_JOIN(rawbuf_tag_optional_, rawbuf_name) + 1){ \
				next_type::iterate_with_depth(this_instance, func_instance, rawbuf_depth); \
			}\
    ...
    struct rawbuf_writer_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name)> : public rawbuf_writer_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name) - 1 >{\
        ...
        rawbuf_writer<local_type> rawbuf_name() { \
            RAWBUF_T* that = (*this)(); \
            offset_type *rawbuf_foffset = that->_.field_offset + 1 + RAW_BUF_JOIN(rawbuf_tag_optional_, rawbuf_name); \
			rawbuf_writer<typename RAWBUF_T::template type_indexer<int,RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name)>::type> result; \
            result.writer = this->writer; \
			char* dp; \
			if(*rawbuf_foffset == 0){ \
...
#define ADD_PACKET_ARRAY_COMMON(rawbuf_input_type, rawbuf_name, size_estimation, init_func, is_rawbuf_ref, visit_func) \
        ...
        static void iterate_with_depth(RAWBUF_This& this_instance, RAWBUF_Func& func_instance, int rawbuf_depth){ \
			...
			if(rawbuf_psize != 0) {\
				...
				if(func_instance(rawbuf_data, *rawbuf_psize, #rawbuf_name, rawbuf_depth)){ \
					...
					char* pdata = (char*)rawbuf_data; \
                    for(array_count_type i = 0; i < *rawbuf_psize; ++i, pdata+=data_real_size){ \
						bool result = func_instance(((local_type*)pdata), 0, rawbuf_depth + 1); /*{}*/\
						if(result){ \
    ...
    struct rawbuf_reader_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name)> : public rawbuf_reader_helper<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name) - 1 >{\
		...
		array_count_type* rawbuf_get_array_count_pointer() { \
            ...
            if(!RAW_BUF_IS_ALIGNED_PTR(rawbuf_psize, rawbuf::rawbuf_alignment<array_count_type>::result)){	    /*2. The location of the length of the optional field should be aligned.*/	\
                this->invalidate(RAW_BUF_ERROR_MSG("Optional field count of elements is not aligned!" RAW_BUF_INFO(ARGS_LIST(rawbuf_input_type), rawbuf_name))); \
                return 0; \
            }\
            return rawbuf_psize; \
		} \
...
#define ADD_IMPL_INIT(rawbuf_input_type, rawbuf_name, count) \
    ...
    struct members_iterator<RAWBUF_T, RAW_BUF_JOIN(rawbuf_tag_, rawbuf_name), false >{ \
        ...
        static const rawbuf_uint sizer         = (count)*rawbuf::rawbuf_property<ARGS_LIST(rawbuf_input_type)>::size_result + prev_type::sizer;\
        static const rawbuf_uint aligned_1x    = prev_type::aligned_1x + tmp_aligned_coefficient*rawbuf::rawbuf_property<ARGS_LIST(rawbuf_input_type)>::aligned_1x; \
        static const rawbuf_uint aligned_2x    = prev_type::aligned_2x + tmp_aligned_coefficient*rawbuf::rawbuf_property<ARGS_LIST(rawbuf_input_type)>::aligned_2x; \
        static const rawbuf_uint aligned_4x    = prev_type::aligned_4x + tmp_aligned_coefficient*rawbuf::rawbuf_property<ARGS_LIST(rawbuf_input_type)>::aligned_4x; \
        static const rawbuf_uint aligned_8x    = prev_type::aligned_8x + tmp_aligned_coefficient*rawbuf::rawbuf_property<ARGS_LIST(rawbuf_input_type)>::aligned_8x; \
		static const rawbuf_uint required_alignment = ((rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result > prev_type::required_alignment ) && (is_required)) ? rawbuf::rawbuf_alignment<ARGS_LIST(rawbuf_input_type)>::result : prev_type::required_alignment; \
    }; \
	template<typename RAWBUF_T>\
